...
namespace GodAndMe.Views
{
    ...
    public partial class IntentionPage : ContentPage
    {
        ...
        public void OnShare(object sender, EventArgs e)
        {
            ...
            try
            {
                string url = ("https://godandme.app/share/" + CryptFile.Encrypt_Legacy(CryptFile.Compress(JsonConvert.SerializeObject(new string[] { item.Person, item.Description, item.Start == null ? string.Empty : ((DateTime)item.Start).ToString("yyyy-MM-dd") }))) + "?lang=" + CommonFunctions.Culture.TwoLetterISOLanguageName).Trim();
